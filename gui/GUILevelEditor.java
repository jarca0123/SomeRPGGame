...
public class GUILevelEditor extends GUI {
    ...
    private int entryPages;
    private int additionalTiles;
    private int tilesInToolbar = 8;
    private int originalOffsetX = 0;
    private int originalOffsetY = 0;
    private HashSet<Tile> selectedTiles = new HashSet<>();
    private int toolId = 1;
    private int lastToolId;
    private int originalDragX;
    private int originalDragY;
    ...
    public void onStart() {
        ...
        for (int i = 0; i < Game.world.tileList.size(); i++) {
            addComponent(new ImageNText(i, 64 + ((i % tilesInToolbar) * 64), Game.GAME_HEIGHT - 64, 64, 64, Game.world.tileList.get(i).image, Game.world.tileList.get(i).name));
        }
        for (int j = 0; j < entries; j++) {
            this.guiComponents.get(j).enabled = false;
            this.guiComponents.get(j).visible = false;
        }
        if (entryPages == 1) {
                    ...
                    if (Game.world.room.roomTiles[roomX][roomY].bounds.intersects(new Rectangle(mouseX - Game.world.offsetX, mouseY - Game.world.offsetY, 1, 1))) {
                        ...
                        if (!SwingUtilities.isRightMouseButton(e)) {
                            if (((GUILevelEditor) Game.gui).toolId == 1) {
                                ...
                                if (addWindow)
                                    gui.window.Window.addGUIWindow(new DataEdit(Game.gui.windows.size() + 1, (Game.GAME_WIDTH / 2) - 50, (Game.GAME_HEIGHT / 2) - 50, 50, 100, Game.world.room.roomTiles[roomX][roomY]));
                            }
                        } else {
                            if (((GUILevelEditor) Game.gui).toolId == 1) {
                                if (Game.world.room.roomTiles[roomX][roomY].bounds.intersects(new Rectangle(mouseX - Game.world.offsetX, mouseY - Game.world.offsetY, 1, 1))) {
    ...
    public void draw(Graphics2D g2d) {
        ...
        g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f));
        if (dragBounds != null)
            g2d.fillRect((int) dragBounds.getX(), (int) dragBounds.getY(), (int) dragBounds.getWidth(), (int) dragBounds.getHeight());
        g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f));
        g2d.setColor(Color.decode("#333333"));
    ...
    private Rectangle generateRectangleThatIDontNeedToRewriteEverytime() {
